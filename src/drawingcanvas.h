...
class DrawingCanvas : public QGraphicsScene
{
  ...
  public:
     ...
     * 		- empty space deselects everything, unless the mouse is dragged which draws a selection
     * box
     * 		- an atom,  the atom is (de)selected, unless the mouse moves which triggers the
     * TempMoveAll
     * mode
     * 		- any other moveable item clicking (de)selects, unless the mouse is moved which triggers
     * TempMove
     * TempMove    - The select mode has clicked an object and requested a move, so only this object
     * is moved
    ...
    {
        return myRotateCursor;
    }
    void serialize(QXmlStreamWriter *writer);
    static DrawingCanvas *
    deserialize(QXmlStreamReader *reader, DrawingInfo *drawingInfo, FileParser *parser);
    QList<Bond *> getBonds()
    {
        return bondsList;
    }
