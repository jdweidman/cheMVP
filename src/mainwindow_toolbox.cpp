...
QWidget *MainWindow::createAppearanceWidget(QMap<QString, QString> *options)
{
    ...
    useFoggingBox->setToolTip(
        tr("Add a fog effect to more distant atoms to emphasize those in the foreground\n0%: no "
    ...
    xRotationBox->setToolTip(
        tr("Type in an angle (in degrees) to rotate the molecule from its initial orientation"));
    yRotationBox->setToolTip(
        tr("Type in an angle (in degrees) to rotate the molecule from its initial orientation"));
    zRotationBox->setToolTip(
        tr("Type in an angle (in degrees) to rotate the molecule from its initial orientation"));
    connect(xRotationBox,
            SIGNAL(textChanged(const QString &)),
            this,
            SLOT(rotateFromInitialCoordinates()));
    connect(yRotationBox,
            SIGNAL(textChanged(const QString &)),
            this,
            SLOT(rotateFromInitialCoordinates()));
    connect(zRotationBox,
            SIGNAL(textChanged(const QString &)),
            this,
            SLOT(rotateFromInitialCoordinates()));
    xLabel = new QLabel("X:");
...
QWidget *MainWindow::createBondsAndAnglesWidget(QMap<QString, QString> *options)
{
    ...
    toggleAngleLabelsButton->setToolTip(tr("Select three or more atoms to toggle the angle markers "
                                           "and labels.  Only angles between bonds will be drawn"));
...
QWidget *MainWindow::createAtomsWidget(QMap<QString, QString> *options)
{
    ...
    atomDrawingStyleButtonGroup->addButton(simpleColoredAtomDrawingButton,
                                           int(DrawingInfo::SimpleColored));
    atomDrawingStyleButtonGroup->addButton(houkMolAtomDrawingButton, int(DrawingInfo::HoukMol));
    atomDrawingStyleButtonGroup->addButton(gradientColoredAtomDrawingButton,
                                           int(DrawingInfo::Gradient));
    ...
    atomDrawingStyleButtonGroup->button(options->value("ATOM_DRAWING_STYLE").toInt())
        ->setChecked(true);
    ...
    toggleAtomNumberSubscriptsButton->setToolTip(
        tr("Add/remove atom numbers as a subscript to the selected atoms"));
    ...
    atomLabelInput->setToolTip(tr("Text entered here will be used as the label for the selected "
                                  "atom(s).  Anything appended after an underscore will be used as "
                                  "a subscript, anything after a carat is a superscript"));
